...
class TrieNode {
 public:
   ...
   */
  bool HasChildren() const { return children_.empty() == false;}
  ...
  std::unique_ptr<TrieNode> *InsertChildNode(char key_char, std::unique_ptr<TrieNode> &&child) { 
    if(HasChild(key_char) || child->GetKeyChar() != key_char)   {return nullptr;}
   ...
   *         node does not exist.
   */
  std::unique_ptr<TrieNode> *GetChildNode(char key_char) { if(!HasChild(key_char)) {return nullptr;} return &children_[key_char]; }
...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    if (cur->get()->IsEndNode()) {latch_.WUnlock(); return false;}
  ...
  bool Remove(const std::string &key) { 
    ...
    for(auto i = key.begin(); i < key.end(); i++){
      if(cur->get()->HasChild(*i)){
        st.push(std::make_tuple(*i, cur));
        cur = cur->get()->GetChildNode(*i);
      } else {
        cur = nullptr;
        while(!st.empty()) {st.pop();}
        //std::cout << "non-exist false" << std::endl;
        latch_.WUnlock();
        return false;
      }
    }
    ...
    while(!st.empty()){
      auto tmp = st.top();
      st.pop();
      char x = std::get<0>(tmp); auto y = std::get<1>(tmp);
      auto y_child = (*y)->GetChildNode(x);
      if(!(*y_child)->HasChildren()){
        (*y)->RemoveChildNode(x);
      }
    }
  ...
  T GetValue(const std::string &key, bool *success) {
    *success = false;
    ...
    if (cur != nullptr && cur->get()->IsEndNode()){
      auto* cur_value = dynamic_cast<TrieNodeWithValue<T> *>(cur->get());
      if(cur_value != nullptr){
        *success = true;

        latch_.RUnlock();
        return cur_value->GetValue();
      }
    }
    latch_.RUnlock();
    return {};
